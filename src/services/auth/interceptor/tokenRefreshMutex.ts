  ...
  refreshPromise = (async () => {
    try {
      ...
      if (isValidToken(tok)) {
        ...
        const currentCompanyId = (api.getState() as RootState)?.auth?.companyId;
        const activeCompanyId = d?.user?.companyId || u?.companyId || currentCompanyId;
        api.dispatch(
          setCredentials({
