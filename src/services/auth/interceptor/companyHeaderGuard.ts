...
import { isValidToken, isValidUUID, needsCompanyId, getStoredAccessToken, getStoredCompanyId } from "../authStorage";
...
export async function validateCompanyHeader(requestUrl: string, api: any): Promise<boolean> {
  ...
  if (isValidToken(token) && needsCompanyId(requestUrl, api.endpoint)) {
    let companyId = state?.auth?.companyId || state?.company?.activeCompany?.id || getStoredCompanyId();
    ...
    companyId = finalState?.auth?.companyId || finalState?.company?.activeCompany?.id || getStoredCompanyId();
