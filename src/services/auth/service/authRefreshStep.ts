...
export async function fetchRefreshAndRetry(rawBaseUrl: string, inMemoryRefreshToken?: string | null) {
  ...
  if (!isValidToken(tokenToUse)) return null;

  try {
    const refreshRes = await fetch(`${rawBaseUrl}/api/v1/auth/refresh`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ refreshToken: tokenToUse.trim(), refresh_token: tokenToUse.trim() }),
      credentials: "include",
    });
    ...
    const retryMeRes = await fetch(`${rawBaseUrl}/api/v1/auth/me`, {
      method: "GET",
      headers: { "Content-Type": "application/json", Authorization: `Bearer ${newToken.trim()}` },
      credentials: "include",
    });
    ...
    return {
      ...
      token: newToken.trim(),
      refreshToken: isValidToken(newRefreshToken) ? newRefreshToken.trim() : tokenToUse.trim(),
    };
  } catch {
    return null;
  }
