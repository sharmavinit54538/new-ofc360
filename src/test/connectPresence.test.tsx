...
describe("OFC360 Connect Real-Time User Presence System", () => {
  ...
  describe("2. Redux Presence State & Selectors", () => {
    it("sets and retrieves user presence by ID and aliases", () => {
      store.dispatch(setUserPresence({ userId: "usr_rubel", user_id: "rubel_123", email: "rubel@ofc360.com", status: "online" }));
  ...
  describe("3. PresenceIndicator Component Verification", () => {
    it("renders green pulsing dot for online status", () => {
      const { container } = renderWithProviders(<PresenceIndicator status="online" withPulse showLabel />);
  ...
  describe("4. ChatList Dynamic Presence Integration", () => {
    it("renders ChatList container with direct conversations", () => {
      renderWithProviders(<ChatList />);
      expect(screen.getByText("Direct Messages")).toBeInTheDocument();
    });
  });

  describe("5. WebSocket Disconnect & Presence Broadcast", () => {
