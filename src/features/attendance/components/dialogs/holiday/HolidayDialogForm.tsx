...
export function HolidayDialogForm(p: {
  holidayTitle: string; setHolidayTitle: (v: string) => void; holidayDate: string; setHolidayDate: (v: string) => void;
  holidayType: HolidayItem["type"]; setHolidayType: (v: HolidayItem["type"]) => void; holidayBranch: string; setHolidayBranch: (v: string) => void;
}) {
  return (
    <div className="space-y-3 py-2">
      ...
      <div className="grid grid-cols-2 gap-2">
        <div><Label className="text-xs">Type</Label><Select value={p.holidayType} onValueChange={(v) => p.setHolidayType(v as HolidayItem["type"])}><SelectTrigger className="h-8 text-xs mt-1"><SelectValue /></SelectTrigger><SelectContent className="text-xs">{HOLIDAY_TYPES.map((t) => (<SelectItem key={t} value={t}>{t}</SelectItem>))}</SelectContent></Select></div>
