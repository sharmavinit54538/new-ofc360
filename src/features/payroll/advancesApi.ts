...
export const advancesApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    getAdvances: builder.query<APIResponse<Advance[]>, PaginationQueryParams | void>({ query: (p) => ({ url: "/v2/payroll/advances", params: p || undefined }), providesTags: (r) => r?.data ? [...r.data.map(({ id }) => ({ type: tag as const, id })), { type: tag, id: "LIST" }] : [{ type: tag, id: "LIST" }] }),
