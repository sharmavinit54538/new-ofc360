...
export const bonusesApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getBonuses: builder.query<APIResponse<Bonus[]>, PaginationQueryParams | void>({ query: (p) => ({ url: "/v2/payroll/bonuses", params: p || undefined }), providesTags: (r) => r?.data ? [...r.data.map(({ id }) => ({ type: tag as const, id })), { type: tag, id: "LIST" }] : [{ type: tag, id: "LIST" }] }),
