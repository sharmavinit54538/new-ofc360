...
export function computeEmployeePayroll(p: any): DetailedPayrollComputation {
  ...
  const net = Math.max(0, gross - totalDed + (p.approvedReimbursement || 0));
  return { employeeId: p.employeeId, employeeName: p.employeeName, annualCtc: p.annualCtc, monthlyCtc: Math.round(p.annualCtc / 12), components: comp, bonusAmount: p.approvedBonus || 0, overtimeAmount: ot, reimbursementAmount: p.approvedReimbursement || 0, grossEarnings: gross, statutoryDeductions: stat, advanceEmiDeduction: p.activeSalaryAdvanceEmi || 0, lopDays: p.lopDays || 0, lopDeduction: lop, totalDeductions: totalDed, netSalary: net };
}