    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-md bg-card border-border/80 shadow-2xl rounded-2xl">
        ...
        <form onSubmit={onSubmit} className="space-y-4 py-2">
          <AddPaymentMethodFields form={form} onChange={onChange} />
          <DialogFooter className="gap-2 sm:gap-0 pt-2"><Button type="button" variant="ghost" size="sm" onClick={() => onOpenChange(false)} className="text-xs">Cancel</Button><Button type="submit" size="sm" disabled={isAdding} className="gradient-bg text-primary-foreground font-semibold text-xs gap-1.5">{isAdding ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Save className="w-3.5 h-3.5" />} Save Payment Method</Button></DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
  );
