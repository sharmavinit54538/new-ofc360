...
export function ComplianceReportTab({ compDash, compRisks, compReadiness, complianceFilings }: any) {
  const compData = compDash?.data?.data ?? compDash?.data;
  const compRisksList = compRisks?.data?.data ?? compRisks?.data;
  const { isLoading, isError } = compDash;
  return (
    <motion.div key="compliance" initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -6 }} className="space-y-6">
      {isLoading ? (
        <div className="p-12 text-center rounded-2xl bg-secondary/20 border border-border/40 space-y-3">
          ...
          <p className="text-xs text-muted-foreground">Loading compliance telemetry...</p>
        </div>
      ) : isError ? (
        ...
        <>
          ...
          <ComplianceRiskAuditCard list={compRisksList} />
        </>
      )}
    </motion.div>
  );
