...
export function BasicInfoNamesRow({ b }: { b: BasicInfoState }) {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
      <div className="space-y-1.5"><Label className="text-xs font-semibold">First Name *</Label><Input placeholder="Jane" value={b.firstName} onChange={(e) => b.setFirstName(e.target.value)} className="bg-secondary/30 text-xs h-10 border-border/60" required /></div>
      <div className="space-y-1.5"><Label className="text-xs font-semibold">Last Name *</Label><Input placeholder="Cooper" value={b.lastName} onChange={(e) => b.setLastName(e.target.value)} className="bg-secondary/30 text-xs h-10 border-border/60" required /></div>
