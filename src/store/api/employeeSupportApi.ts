...
export const employeeSupportApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    ...
    updateV2EmployeeSupportTicketsTicketId: builder.mutation<ApiResponse<any>, any>({
      query: (data) => ({
        url: typeof data === 'string' || typeof data === 'number' ? `/api/v2/employee-support/tickets/${data}` : typeof data === 'object' && data?.id ? `/api/v2/employee-support/tickets/{ticket_id}` : '/api/v2/employee-support/tickets/{ticket_id}',
        method: 'PATCH',
        body: typeof data === 'object' ? data : undefined,
      }),
      invalidatesTags: ['EmployeeSupport'],
    ...
    createV2WellnessAnonymousChatsSessionIdMessages: builder.mutation<ApiResponse<any>, any>({
      query: (data) => ({
        url: typeof data === 'string' || typeof data === 'number' ? `/api/v2/wellness/anonymous-chats/${data}/messages` : typeof data === 'object' && data?.id ? `/api/v2/wellness/anonymous-chats/{session_id}/messages` : '/api/v2/wellness/anonymous-chats/{session_id}/messages',
        method: 'POST',
        body: typeof data === 'object' ? data : undefined,
      }),
      invalidatesTags: ['EmployeeSupport'],
