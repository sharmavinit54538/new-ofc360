...
export const announcementsApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getAnnouncementsId: builder.query<ApiResponse<any>, any>({
      query: (params) => ({
        url: typeof params === 'string' || typeof params === 'number' ? `/api/v1/announcements/${params.id}` : typeof params === 'object' && params?.id ? `/api/v1/announcements/${params.id}` : '/api/v1/announcements/{id}',
        params: typeof params === 'object' ? params : undefined,
      }),
      providesTags: ['Event'],
    }),
