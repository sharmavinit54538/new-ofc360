...
export const announcementsApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    ...
    getAnnouncementsId: builder.query<ApiResponse<any>, any>({
      query: (params) => ({
        url: typeof params === 'string' || typeof params === 'number' ? `/api/v1/announcements/${params}` : typeof params === 'object' && params?.id ? `/api/v1/announcements/${params.id}` : '/api/v1/announcements/{id}',
        params: typeof params === 'object' ? params : undefined,
      }),
      providesTags: ['Event'],
    }),
    updateAnnouncementsId: builder.mutation<ApiResponse<any>, any>({
      query: (data) => ({
        url: typeof data === 'string' || typeof data === 'number' ? `/api/v1/announcements/${data}` : typeof data === 'object' && data?.id ? `/api/v1/announcements/${data.id}` : '/api/v1/announcements/{id}',
        method: 'PUT',
        body: typeof data === 'object' ? data : undefined,
      }),
      invalidatesTags: ['Event'],
    ...
    updateAnnouncementsIdPublish: builder.mutation<ApiResponse<any>, any>({
      query: (data) => ({
        url: typeof data === 'string' || typeof data === 'number' ? `/api/v1/announcements/${data}/publish` : typeof data === 'object' && data?.id ? `/api/v1/announcements/${data.id}/publish` : '/api/v1/announcements/{id}/publish',
        method: 'PATCH',
        body: typeof data === 'object' ? data : undefined,
      }),
      invalidatesTags: ['Event'],
    ...
    updateAnnouncementsIdArchive: builder.mutation<ApiResponse<any>, any>({
      query: (data) => ({
        url: typeof data === 'string' || typeof data === 'number' ? `/api/v1/announcements/${data}/archive` : typeof data === 'object' && data?.id ? `/api/v1/announcements/${data.id}/archive` : '/api/v1/announcements/{id}/archive',
        method: 'PATCH',
        body: typeof data === 'object' ? data : undefined,
      }),
      invalidatesTags: ['Event'],
