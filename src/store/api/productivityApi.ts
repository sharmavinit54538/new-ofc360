...
export const productivityApi = baseApi.injectEndpoints({
  endpoints: (builder) => ({
    ...
    createV2ProductivityForecastEmployeeId: builder.mutation<ApiResponse<any>, any>({
      query: (data) => ({
        url: typeof data === 'string' || typeof data === 'number' ? `/api/v2/productivity/forecast/${data.employee_id}` : typeof data === 'object' && data?.id ? `/api/v2/productivity/forecast/{employee_id}` : '/api/v2/productivity/forecast/{employee_id}',
        method: 'POST',
        body: typeof data === 'object' ? data : undefined,
      }),
      invalidatesTags: ['Productivity'],
